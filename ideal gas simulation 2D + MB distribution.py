...
particles = [
    [
        i,
        np.random.uniform([-1, -1], [1, 1], size=2),
        v_0 * np.array([np.cos(np.random.uniform(-1, 1) * 2 * np.pi), np.sin(np.random.uniform(-1, 1) * 2 * np.pi)]),
        1e-2,
        m,
        "green",
    ]
...
dist_ax.set_ylim(0, max(Nbin * N * (m / (2 * np.pi * kB * T)) ** (3 / 2) * 4 * np.pi * v_bins ** 2 * np.exp(-m * v_bins ** 2 / (2 * kB * T))))
...
dist_ax.plot(np.arange(0, 2 * v_0, 1), Nbin/1.1 * N * (m / (2 * np.pi * kB * T)) ** (3 / 2) * 4 * np.pi * np.arange(0, 2 * v_0, 1) ** 2 * np.exp(-m * np.arange(0, 2 * v_0, 1) ** 2 / (2 * kB * T)), color="black")
...
ani = FuncAnimation(fig, update, frames=range(250), init_func=init, blit=True, interval=1, repeat=False)

# Sauvegarder l'animation en tant que fichier vidéo MP4
